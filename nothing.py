...
player_stats = leaguedashplayerstats.LeagueDashPlayerStats(season='2023-24').get_data_frames()[0][['PTS', 'FTA', 'FGA']]
LEAGUE_TS_PCT = round(50*player_stats['PTS'].mean()/(player_stats['FGA'].mean()+.44*player_stats['FTA'].mean()),2)
...
def get_expected_pts(comparison,shotchart,game,player_id):
    x_pts=0
    ...
    for index,(x,y,res,value) in shotchart.iterrows():
        ...
        st.write(x,y,res,comparison[hex])
        if np.isnan(comparison[hex]['player_fg']):
            x_pts += value*comparison[hex]['global_fg']
        else:
...
def get_player_season_averages(season,player_id):

    season_df = playergamelog.PlayerGameLog(player_id=selected_player_id, season=season).get_data_frames()[0]
...
TS% = {TS_PCT}% | TS+ = {round(100*TS_PCT/LEAGUE_TS_PCT,0)}"""
    

    return stats_str

def get_player_game_stats(game_id, player_id, season):

    season_df = playergamelog.PlayerGameLog(player_id=player_id, season=season).get_data_frames()[0]
    game_df = season_df[season_df['Game_ID']==game_id]

    pts = game_df['PTS'].values[0]
    _2ptFG_PCT = int(100*round(game_df['FG_PCT'].values[0],2))
    _3ptFG_PCT = int(100*round(game_df['FG3_PCT'].values[0],2))
    _FT_PCT = int(100*round(game_df['FT_PCT'].values[0],2))
    minutes = game_df['MIN'].values[0]

    TS_PCT = round(50*(pts)/(game_df['FGA'].values[0]+game_df['FTA'].values[0]),1)

    stats_str = f"""{pts} PTS on {_2ptFG_PCT}/{_3ptFG_PCT}/{_FT_PCT} in {minutes}min 
TS% = {TS_PCT}% | TS+ = {round(100*TS_PCT/LEAGUE_TS_PCT,0)}"""
...
selected_player = st.selectbox("",sorted(df['PLAYER_NAME'].unique()),index=6,placeholder="Select a player ...")
...
games_df['Matchup + Date'] = games_df['MATCHUP'].apply(lambda x: x[4:]) + " - " + games_df['GAME_DATE']

game = st.selectbox(
    "Chose a Game to plot a specific shotchart",
    games_df['Matchup + Date'],
    index=0,
    placeholder="Select game ...",
)

selected_game = games_df[games_df['Matchup + Date']==game]
game_id = selected_game['Game_ID'].values[0]

Team_ID = teams.find_team_by_abbreviation(selected_game['MATCHUP'].values[0][:3])['id']
...
shotchart=shotchart[['LOC_X','LOC_Y','SHOT_MADE_FLAG', 'SHOT_TYPE']]
shotchart['SHOT_TYPE'] = shotchart['SHOT_TYPE'].apply(lambda x: x[0])

shotchart['LOC_X'] = shotchart['LOC_X'].apply(lambda x:-x)
...
ax2.set_ylim(-50,335)
ax2.set_axis_off()
ax2.set_title(f"{selected_player} {game}",fontdict={'fontsize': 7})

ax3.set_axis_off()

video_link = f"https://www.nba.com/stats/events?CFID=&CFPARAMS=&ContextMeasure=FGA&EndPeriod=0&EndRange=28800&GameID={game_id}&PlayerID={selected_player_id}&Season=2024-25&TeamID={Team_ID}&flag=3&sct=plot"
...
try:
    ...
    st.write(f"{get_expected_pts(comparison, shotchart,None,None)} xPTS")
    ...
    figure.text(0.3, 0.03, get_player_season_averages("2024-25",selected_player_id), horizontalalignment="center",fontdict={'fontsize': 7})
    figure.text(.685, 0.03, get_player_game_stats(game_id,selected_player_id,"2024-25"), horizontalalignment="center",fontdict={'fontsize': 7})
...
# 2 conteneurs pour avoir les vidéos de tous les tirs, et de ceux du match sur la même ligne. En dessous de la figure pyplot.
st.write("[Videos](%s)" % video_link)
...
st.download_button(
    label="PHOTO",
    data=buffer,
    file_name=f"{selected_player} shot chart (season & {game}).png",
)
...
- TS+ : Measure of a player's efficiency relative to league average. 100 is average, 102 is 2% better than average, 90 is 10% worse.

- xPTS : How many points a player "should have scored" based on his shot selection and season averages in these zones.

- Hotness emoji : Measure of a specific performance relative to the player's season averages. Difference in points scored and expected
points, with a weight put on volume. Meaning if a player has better efficiency than his average suggest he is hot, but if he has it on high volume he is hotter than if he had it on low volume.
""")
